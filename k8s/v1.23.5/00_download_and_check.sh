...
if [ ! -f 'etc_modules-load.d_k8s.conf' ];then
    wget "${baseUrl}k8s/v1.23.5/etc_modules-load.d_k8s.conf"
fi

if [ ! -f 'etc_sysctl.d_k8s.conf' ];then
    wget "${baseUrl}k8s/v1.23.5/etc_sysctl.d_k8s.conf"
fi

if [ ! -f 'etc_yum.repos.d_kubernetes.repo' ];then
    wget "${baseUrl}k8s/v1.23.5/etc_yum.repos.d_kubernetes.repo"
fi

if [ ! -f 'etc_docker_daemon.json' ];then
    wget "${baseUrl}k8s/v1.23.5/etc_docker_daemon.json"
fi

if [ ! -f 'kubeadm-config.yaml' ];then
    wget "${baseUrl}k8s/v1.23.5/kubeadm-config.yaml"
fi
...
if [ ! -f '01_new_server_master_init.sh' ];then
    wget "${baseUrl}k8s/v1.23.5/01_new_server_master_init.sh"
fi

if [ ! -f '02_new_server_node_init.sh' ];then
    wget "${baseUrl}k8s/v1.23.5/02_new_server_node_init.sh"
fi
